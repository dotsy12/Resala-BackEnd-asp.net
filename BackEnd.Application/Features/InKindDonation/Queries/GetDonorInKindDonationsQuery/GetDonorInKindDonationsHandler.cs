...
{
    public class GetDonorInKindDonationsHandler
      : IRequestHandler<GetDonorInKindDonationsQuery, Result<IReadOnlyList<InKindDonationDto>>>
    {
        private readonly IInKindDonationRepository _repo;

        public GetDonorInKindDonationsHandler(IInKindDonationRepository repo)
            => _repo = repo;

        public async Task<Result<IReadOnlyList<InKindDonationDto>>> Handle(
            GetDonorInKindDonationsQuery request, CancellationToken ct)
        {
            var donations = await _repo.GetByDonorIdAsync(request.DonorId, ct);
